...
public class IP2Proxy {
    ...
    public int Open(String DatabasePath, IOModes IOMode) throws IOException {
        CmAntiBot plugin = CmAntiBot.getPlugin(CmAntiBot.class);
        if (_DBType == 0) {
            _IPDatabasePath = plugin.getDataFolder().getAbsolutePath() + "/" + DatabasePath;
    ...
    public ProxyResult ProxyQuery(String IPAddress) throws IOException {
        return ProxyQuery(IPAddress, Modes.IS_PROXY);
    }
